...
def cal_general_stats(df):
    ...
    df_data["Média 6M diferença Mensal"] = (
        df_data["Diferença Mensal Abs."].rolling(6).mean()
    )
    df_data["Média 12M diferença Mensal"] = (
        df_data["Diferença Mensal Abs."].rolling(12).mean()
    )
    df_data["Média 24M diferença Mensal"] = (
        df_data["Diferença Mensal Abs."].rolling(24).mean()
    )
    ...
    df_data["Evolução 24M Total"] = (
        df_data["Valor"].rolling(24).apply(lambda x: x[-1] - x[0])
    )
    ...
    df_data["Evolução 24M Relativa"] = (
        df_data["Valor"].rolling(24).apply(lambda x: x[-1] / x[0] - 1)
    )
...
def main_metas():
    ...
    custos_fixos = col1.number_input("Custos Finais", min_value=0.0, format="%.2f")
...
if file_upload:
    ...
    colums_config = {
        ...
        "Média 6M diferença Mensal": st.column_config.NumberColumn(
            "Média 6M diferença Mensal", format="R$ %.2f"
        ),
        "Média 12M diferença Mensal": st.column_config.NumberColumn(
            "Média 12M diferença Mensal", format="R$ %.2f"
        ),
        "Média 24M diferença Mensal": st.column_config.NumberColumn(
            "Média 24M diferença Mensal", format="R$ %.2f"
        ),
    ...
    with st.expander("Metas"):
        ...
        with tab_data_meta:
            ...
            st.dataframe(meses, column_config=colums_config_meta)
